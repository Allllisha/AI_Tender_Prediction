#!/usr/bin/env python
"""
Azure PostgreSQLに四国地方の入札データを追加するスクリプト
"""
import psycopg2
import os
...
def add_shikoku_tenders():
    ...
    try:
        # 既存の四国データを削除（重複を避けるため）
        cursor.execute("""
            DELETE FROM tenders_open 
            WHERE prefecture IN ('徳島県', '香川県', '愛媛県', '高知県')
            AND source = 'shikoku_mock'
        """)
        deleted_count = cursor.rowcount
        if deleted_count > 0:
            print(f"  既存の四国モックデータ {deleted_count} 件を削除しました")
        ...
        print(f"📊 データベース全体の件数: {total_in_db} 件")
...
if __name__ == "__main__":
    print("🔄 Azure PostgreSQLに四国地方の入札データを追加中...")
    add_shikoku_tenders()